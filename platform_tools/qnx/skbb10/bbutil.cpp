...
EGLConfig bbutil_choose_config(EGLDisplay egl_disp, enum RENDERING_API api) {
    ...
    if (egl_num_configs == 0) {
        SkDebugf("eglGetConfigs: could not find a configuration\n");
        return egl_conf;
    }

    egl_configs = new EGLConfig[egl_num_configs];
    if (egl_configs == NULL) {
        SkDebugf("could not allocate memory for %d EGL configs\n", egl_num_configs);
        return egl_conf;
    }
    ...
    if (egl_conf == (EGLConfig)0) {
        SkDebugf("bbutil_choose_config: could not find a matching configuration\n");
    }
...
bbutil_swap(int index) {
    if (use_native) {
        static const int rect[4] = {0, 0, atoi(getenv("WIDTH")), atoi(getenv("HEIGHT"))};
