...
int init() {
	...
	desc.fWidth = surface_width;
	desc.fHeight = surface_height;
	desc.fStencilBits = 8;
	desc.fConfig = kRGBA_8888_PM_GrPixelConfig;
	desc.fRenderTargetHandle = 0;

	GrRenderTarget *renderTarget = static_cast<GrRenderTarget*>(context->createPlatformRenderTarget(desc));

    device = new SkGpuDevice(context, renderTarget);
...
#if USE_SKIA_OPENGL
    static_cast<SkGpuDevice*>(device)->flush();
    bbutil_swap(0);
#else
