...
void SkBitmapProcShader::shadeSpan(int x, int y, SkPMColor dstC[], int count) {
    ...
    // Interim crash prevention until upstream Skia actually reads the pixels
    // from a texture-only bitmap.
    if (!state.fBitmap->getPixels()) {
        memset(buffer, 0, sizeof(buffer[0]) * BUF_MAX);
        return;
    } // end of interim crash prevention
