...
typedef std::map<std::string, unsigned> SkFCMapType;
static SkFCMapType *global_fc_map_;
static SkFCMapType *global_fc_map()
{
    if (!global_fc_map_)
        global_fc_map_ = new SkFCMapType;
    return global_fc_map_;
}
typedef std::map<unsigned, std::string> SkFCMapInvertedType;
static SkFCMapInvertedType *global_fc_map_inverted_;
static SkFCMapInvertedType *global_fc_map_inverted()
{
    if (!global_fc_map_inverted_)
        global_fc_map_inverted_ = new SkFCMapInvertedType;
    return global_fc_map_inverted_;
}
typedef std::map<uint32_t, SkTypeface *> SkFCTypefacesType;
static SkFCTypefacesType *global_fc_typefaces_;
static SkFCTypefacesType *global_fc_typefaces()
{
    if (!global_fc_typefaces_)
        global_fc_typefaces_ = new SkFCTypefacesType;
    return global_fc_typefaces_;
}
...
{
    int value = global_fc_map_next_id;

    // If we somehow manage to wrap back to 0, find the next free value.
    SkFCTypefacesType *typefaces = global_fc_typefaces();
    while(typefaces->find(value) != typefaces->end())
        value++;

    global_fc_map_next_id = value + 1;
...
public:
    ...
    virtual ~FontConfigTypeface() {
        ...
        std::map<uint32_t, SkTypeface *>::iterator it;
        it = global_fc_typefaces()->find(uniqueID());
        global_fc_typefaces()->erase(it);
    }

    virtual SkStream* openStream() { return NULL; };
    virtual const char* getUniqueString() { return NULL; };
};

class StreamTypeface : public FontConfigTypeface {
public:
    StreamTypeface(Style style, SkStream* stream, uint32_t id)
    : FontConfigTypeface(style, id) {
        SkAutoMutexAcquire ac(global_fc_map_lock);
        stream->ref();
        fStream = stream;
    }
...
static unsigned FileIdFromFilename(const char* filename)
{
    ...
    SkFCMapType *fcMap = global_fc_map();
    SkFCMapInvertedType *fcMapInverted = global_fc_map_inverted();
    SkFCMapType::const_iterator i = fcMap->find(filename);
    if (i == fcMap->end()) {
        const unsigned fileid = NextId();
        (*fcMap)[filename] = fileid;
        (*fcMapInverted)[fileid] = filename;
        return fileid;
    } else {
        return i->second;
    }
                                       ...
                                       SkTypeface::Style style)
{
    ...
    if (familyFace) {
        ...
        SkFCMapInvertedType *fcMapInverted = global_fc_map_inverted();
        SkFCMapInvertedType::const_iterator i = fcMapInverted->find(fileid);
        if (i == fcMapInverted->end())
            return NULL;
        ...
        face_match = FcPatternBuild(0, FC_FAMILY, FcTypeString, "Arial", (char *) 0);
    ...
    SkFCTypefacesType *typefaces = global_fc_typefaces();
    SkFCTypefacesType::iterator it = typefaces->find(id);
    if (it != typefaces->end()) {
        it->second->ref();
        return it->second;
    }
...
{
    int fileId = NextId();
    int uniqueId = FileIdAndStyleToUniqueId(fileId, SkTypeface::kNormal);

    //TODO: We need to determine the style from the typeface otherwise we just pass normal.
    StreamTypeface * typeface = SkNEW_ARGS(StreamTypeface, (SkTypeface::kNormal, stream, uniqueId));
    {
        SkAutoMutexAcquire ac(global_fc_map_lock);
        (*global_fc_typefaces())[uniqueId] = typeface;
    }
    return typeface;
}
...
SkStream* SkFontHost::OpenStream(uint32_t id)
{
    ...
    //Check to see if there was already a typeface with an open stream.
    SkFCTypefacesType *typefaces = global_fc_typefaces();
    SkFCTypefacesType::iterator it = typefaces->find(id);
    if (it != typefaces->end()) {
        SkStream *result = ((FontConfigTypeface*)it->second)->openStream();
        if(result)
            return result;
    }
...
void SkFontHost::Serialize(const SkTypeface* face , SkWStream* stream) {
    char name[PATH_MAX];
    size_t len = GetFileName(face->uniqueID(), name, PATH_MAX, NULL);
...
SkTypeface* SkFontHost::Deserialize(SkStream* stream) {
	SkDebugf("%d: SkFontHost::Deserialize(SkStream*)\n", __LINE__);
	...
	if (len > 0) {
		...
		SkTypeface::Style tstyle = (SkTypeface::Style)(style & SkTypeface::kBoldItalic);

		const unsigned fileid = FileIdFromFilename(str.c_str());
		const unsigned id = FileIdAndStyleToUniqueId(fileid, tstyle);

		SkDebugf("%s(): fname:  %s, fileid:  %d, fc unique id:  %d\n", __FUNCTION__, str.c_str(), fileid, id);

		SkTypeface* typeface = SkNEW_ARGS(FontConfigTypeface, (tstyle, id));

		SkASSERT(typeface != NULL);
		{
			SkAutoMutexAcquire ac(global_fc_map_lock);
