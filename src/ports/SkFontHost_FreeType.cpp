...
void SkScalerContext_FreeType::generateAdvance(SkGlyph* glyph) {
    // We may not want to use FT_Get_Advance here because for some complex text
    // the advance is not correct enough. For some Arabic glyphs, the
    // difference between the advance generated by FT_Get_Advance and
    // the advance generated by FT_Glyph can be about 10%. This difference
    // is not acceptable for use case in which it measure text first
    // and draw it later.
#if defined(FT_ADVANCES_H) && defined(USE_CACHED_ADVANCES)
...
void SkScalerContext_FreeType::generateFontMetrics(SkPaint::FontMetrics* mx,
    ...
    ys[6] = os2 ? os2->usWinAscent : 0;
    ys[7] = os2 ? os2->usWinDescent : 0;
    ...
    if (mx) {
        ...
        mx->fWinAscent = pts[6].fX;
        mx->fWinDescent = pts[7].fX;
        mx->fXMin = xmin;
        mx->fXMax = xmax;
    ...
    if (my) {
        ...
        my->fWinAscent = pts[6].fY;
        my->fWinDescent = pts[7].fY;
        my->fXMin = xmin;
        my->fXMax = xmax;
