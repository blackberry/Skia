...
// FIXME: Currently, the driver only supports reading back the full buffer.
// Here is to provide a workaround to always read back the whole buffer
// and then extract the requested portion. We need to revert this hack
// once the driver bug is fixed. See PR# ?
#define FULL_BUFFER_FOR_GL_READ_PIXELS 1
...
void SkGpuDevice::makeRenderTargetCurrent() {
	if (((GrGpuGL*)fContext->getGpu())->glInterface()->fmakeCurrent)
		((GrGpuGL*)fContext->getGpu())->glInterface()->fmakeCurrent(fEGLContext);
...
#if FULL_BUFFER_FOR_GL_READ_PIXELS
    if (x || y || this->width() != bitmap.width() || this->height() != bitmap.height()) {
        SkBitmap tmpBitmap;
        tmpBitmap.setConfig(bitmap.config(), this->width(), this->height());
        if (!tmpBitmap.allocPixels()) {
            return false;
        }

        SkAutoLockPixels tmpLock(tmpBitmap);
        GrPixelConfig config;
        config = config8888_to_gr_config(config8888);
        if (!fContext->readRenderTargetPixels(fRenderTarget, 0, 0, this->width(), this->height(),
                                              config, tmpBitmap.getPixels(), tmpBitmap.rowBytes())) {
            return false;
        }

        SkAutoLockPixels dstLock(bitmap);
        const char* srcP = reinterpret_cast<const char*>(tmpBitmap.getAddr32(x, y));
        char* dstP = reinterpret_cast<char*>(bitmap.getPixels());
        // Extracts the requested portion.
        size_t bytesToCopy = bitmap.width() * bitmap.bytesPerPixel();
        for (int i = 0; i < bitmap.height(); i++) {
            memcpy(dstP, srcP, bytesToCopy);
	    srcP += tmpBitmap.rowBytes();
	    dstP += bitmap.rowBytes();
        }
