...
void SkGpuDevice::initFromRenderTarget(GrContext* context, 
    ...
    fEGLContext = eglGetCurrentContext();
...
: SkDevice(config, width, height, false /*isOpaque*/) {
    ...
    if (fContext->getGpu()->renderTargetCount() < fContext->getGpu()->maxRenderTargetCount())
    ...
    if (NULL != fTexture) {
        ...
        GrPrintf("--- failed to create gpu-offscreen [%d %d], falling back to on-screen\n",
        ...
        fRenderTarget = static_cast<GrRenderTarget*>(fContext->createPlatformSurface(desc));
    }
}
...
void SkGpuDevice::makeRenderTargetCurrent() {
    ...
    if (context != fEGLContext) fContext->makeCurrent();
...
bool SkGpuDevice::skPaint2GrPaintShader(const SkPaint& skPaint,
    ...
    if (bitmap.width() > fContext->getMaxTextureSize() ||
        bitmap.height() > fContext->getMaxTextureSize()) {
        grPaint->fFallback = true;
        return false;
    }
...
static void initializeBitmap(SkBitmap& bitmap, const SkGpuDevice* device) {
    bitmap.setConfig(SkBitmap::kARGB_8888_Config, device->width(), device->height());
    bitmap.allocPixels();
    bitmap.eraseColor(0);
}

static void drawBitmapToGpu(const SkBitmap& bitmap, SkGpuDevice* device, const SkDraw& draw) {
    SkMatrix matrix;
    matrix.reset();
    ...
    // Need to reset the matrix here then restore it
    convert_matrixclip(device->context(), matrix, clipstack, *draw.fClip, device->getOrigin());
    device->drawBitmap(draw, bitmap, NULL, matrix, paint);
    convert_matrixclip(device->context(), *draw.fMatrix, *draw.fClipStack, *draw.fClip, device->getOrigin());
}
...
void SkGpuDevice::drawPaint(const SkDraw& draw, const SkPaint& paint) {
    ...
    if (!this->skPaint2GrPaintShader(paint,
                                     ...
                                     *draw.fMatrix,
                                     &grPaint,
                                     true)) {
        if (grPaint.fFallback) {
            // Failed to create shader, fallback to software
            SkBitmap bitmap;
            initializeBitmap(bitmap, this);
            SkAutoCanvasCreator c(bitmap, *draw.fMatrix, *draw.fClip);
            c.canvas()->drawPaint(paint);
            drawBitmapToGpu(bitmap, this, draw);
        }
        return;
    }
...
SkDevice* SkGpuDeviceFactory::newDevice(SkCanvas*, SkBitmap::Config config,
    ...
    if (isLayer) {
        if (height < fContext->getGpu()->minRenderTargetHeight() ||
            width < fContext->getGpu()->minRenderTargetWidth()){
            GrPrintf("--- size too small [%d %d], falling back to software\n",
                     width, height);
            return SkNEW_ARGS(SkDevice, (config, width, height, isOpaque));
        }

        SkGpuDevice* device = SkNEW_ARGS(SkGpuDevice, (fContext, config, width, height));

        if (!device->getLayerTextureHandle()) {
            // We failed to create a texture, revert to SkDevice
            delete device;
            return SkNEW_ARGS(SkDevice, (config, width, height, isOpaque));
        }
        return device;
