...
void GrGpuGLShaders::setupGeometry(int* startVertex,
    ...
    *startVertex = 0;
    ...
    if (newCoverageOffset > 0) {
        GrGLvoid* coveragePtr = (char*)fHWGeometryState.fVertexBuffer->lockPtr() + vertexOffset + newCoverageOffset;
        int idx = GrGLProgram::CoverageAttributeIdx();
        if (oldCoverageOffset <= 0) {
            GL_CALL(EnableVertexAttribArray(idx));
            GL_CALL(VertexAttribPointer(idx, 4, GR_GL_UNSIGNED_BYTE,
                                        true, newStride, coveragePtr));
        } else {
            GL_CALL(VertexAttribPointer(idx, 4, GR_GL_UNSIGNED_BYTE,
                                        true, newStride, coveragePtr));
        }
    ...
    if (newEdgeOffset > 0) {
        GrGLvoid* edgePtr = (char*)fHWGeometryState.fVertexBuffer->lockPtr() + vertexOffset + newEdgeOffset;
        int idx = GrGLProgram::EdgeAttributeIdx();
        if (oldEdgeOffset <= 0) {
            GL_CALL(EnableVertexAttribArray(idx));
            GL_CALL(VertexAttribPointer(idx, 4, scalarType,
                                        false, newStride, edgePtr));
        } else {
            GL_CALL(VertexAttribPointer(idx, 4, scalarType,
                                        false, newStride, edgePtr));
        }
