...
void GrGpuGL::onWriteTexturePixels(GrTexture* texture,
    ...
    if (glTex->eglImage()) {
        // The EGLImages are only used for the glyph atlas (the only code that
        // requests a texture with kDynamicUpdate_GrTextureFlagBit). It wants
        // a GL_ALPHA texture but EGLImages can only do GL_RGBA. We set all
        // channels to the alpha value to avoid endianess issues.
        for (int i = 0; i < height; ++i) {
            char* row = (char*)glTex->eglImage()->data() + glTex->eglImage()->stride()*(i+top);
            char* s = (char*)buffer + i*width;
...
GrTexture* GrGpuGL::onCreateTexture(const GrTextureDesc& desc,
    ...
    if (this->glInterface()->fcreateEGLImage) {
		if (desc.fFlags & kDynamicUpdate_GrTextureFlagBit) {
			GL_CALL_RET(eglImage, createEGLImage(glTexDesc.fWidth, glTexDesc.fHeight));
		}
    }
    if (eglImage) {
        GL_CALL(EGLImageTargetTexture2DOES(GR_GL_TEXTURE_2D, eglImage->eglImage()));
    }
    else if (!this->uploadTexData(glTexDesc, true, 0, 0,
    ...
    if (renderTarget) {
...
#endif
        if (fRenderTargetCnt > fMaxRenderTargetCnt ||
            !this->createRenderTargetObjects(glTexDesc.fWidth,
                                             glTexDesc.fHeight,
                                             glTexDesc.fTextureID,
                                             &glRTDesc)) {
            GL_CALL(DeleteTextures(1, &glTexDesc.fTextureID));
        ...
        tex = new GrGLTexture(this, glTexDesc, eglImage);
    }
