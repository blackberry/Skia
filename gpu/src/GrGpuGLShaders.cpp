...
GrGpuGLShaders::GrGpuGLShaders(GrPlatform3DContext platformContext)
  : INHERITED(platformContext)
{
...
void GrGpuGLShaders::setupGeometry(int* startVertex,
    ...
    *startVertex = 0;
    ...
    int posIdx = GrGLProgram::PositionAttributeIdx();
    GR_GL(VertexAttribPointer(posIdx, 2, scalarType, false, newStride,
                              (char*)fHWGeometryState.fVertexBuffer->lockPtr() + vertexOffset));
    fHWGeometryState.fVertexOffset = vertexOffset;

    for (int t = 0; t < kMaxTexCoords; ++t) {
        if (newTexCoordOffsets[t] > 0) {
            GrGLvoid* texCoordPtr = (char*)fHWGeometryState.fVertexBuffer->lockPtr() + vertexOffset + newTexCoordOffsets[t];
    ...
    if (newColorOffset > 0) {
        GrGLvoid* colorPtr = (char*)fHWGeometryState.fVertexBuffer->lockPtr() + vertexOffset + newColorOffset;
...
void GrGpuGLShaders::buildProgram(GrPrimitiveType type) {
    ...
    for (int s = 0; s < kNumStages; ++s) {
        ...
        if (stage.isEnabled()) {
            ...
            if (!texture->getTexParams().fUseGpuRepeat) {
                if (texture->getTexParams().fWrapS == GR_GL_REPEAT)
                    stage.fWrapModeHorizontal = GrSamplerState::kRepeat_WrapMode;
                if (texture->getTexParams().fWrapT == GR_GL_REPEAT)
                    stage.fWrapModeVertical = GrSamplerState::kRepeat_WrapMode;
            } else {
                stage.fWrapModeHorizontal = GrSamplerState::kClamp_WrapMode;
                stage.fWrapModeVertical = GrSamplerState::kClamp_WrapMode;
            }
