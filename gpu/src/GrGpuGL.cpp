...
GrGpuGL::GrGpuGL(GrPlatform3DContext platformContext) {
    ...
    fPlatformContext = platformContext;
    ...
    if (GR_GL_SUPPORT_ES) {
       ...
       } else if (has_gl_extension("GL_IMG_multisampled_render_to_texture")) {
    ...
    fRenderTargetCnt = 0;
    fMaxRenderTargetCnt = MAX_FBO_CNT;
}
...
GrTexture* GrGpuGL::onCreateTexture(const GrTextureDesc& desc,
    ...
    GrGLPlatform3DContext* platformContext = (GrGLPlatform3DContext*)fPlatformContext;
    if (desc.fFlags & kDynamicUpdate_GrTextureFlagBit && platformContext && (eglImage = platformContext->createEGLImage(glDesc.fAllocWidth, glDesc.fAllocHeight))) {
        GR_GL(EGLImageTargetTexture2DOES(GR_GL_TEXTURE_2D, eglImage->eglImage()));
    }
    else if (kIndex_8_GrPixelConfig == desc.fFormat &&
        supports8BitPalette()) {
    ...
    if (renderTarget) {
        ...
        // The IMG multisampling extension doesn't require this code path though.
        if (samples > 1 && kNone_MSFBO != fMSFBOType && kIMG_MSFBO != fMSFBOType) {
            fRenderTargetCnt++;
...
#endif
            if (samples > 1 && fMSFBOType == kIMG_MSFBO) {
                GR_GL(FramebufferTexture2DMultisample(GR_GL_FRAMEBUFFER,
                                            GR_GL_COLOR_ATTACHMENT0,
                                            GR_GL_TEXTURE_2D,
                                            glDesc.fTextureID, 0, samples));
            } else {
...
#endif
    GrGLTexture* tex = new GrGLTexture(this, glDesc, rtIDs, DEFAULT_PARAMS, eglImage);
...
GrVertexBuffer* GrGpuGL::onCreateVertexBuffer(uint32_t size, bool dynamic) {
    ...
    if (ptr) {
        GrGLVertexBuffer* vertexBuffer = new GrGLVertexBuffer(this, ptr, size, dynamic);
...
GrIndexBuffer* GrGpuGL::onCreateIndexBuffer(uint32_t size, bool dynamic) {
    void* ptr = malloc(size);
    if (ptr) {
        GrIndexBuffer* indexBuffer = new GrGLIndexBuffer(this, ptr, size, dynamic);
...
void GrGpuGL::onGpuDrawIndexed(GrPrimitiveType type,
    ...
    GrGLvoid* indices = (char*)fHWGeometryState.fIndexBuffer->lockPtr() + sizeof(uint16_t)*startIndex;
...
void GrGpuGL::resolveRenderTarget(GrGLRenderTarget* rt) {
    ...
    if (rt->needsResolve()) {
        ...
        if (kAppleES_MSFBO == fMSFBOType || kIMG_MSFBO == fMSFBOType) {
...
bool GrGpuGL::flushGLStateCommon(GrPrimitiveType type) {
    ...
    for (int s = 0; s < kNumStages; ++s) {
        ...
        if (this->isStageEnabled(s)) {
            ...
            // We don't set GL_REPEAT on the texture as we do this in the shader
            // ourselves, unless the texture is a power of two
            newTexParams.fWrapS =
            ...
            if ((GrIsPow2(nextTexture->width()) && GrIsPow2(nextTexture->height()))
                || npotTextureTileSupport()) {
                newTexParams.fUseGpuRepeat = true;
                ...
                newTexParams.fUseGpuRepeat = false;
            }
...
void GrGpuGL::notifyRenderTargetDelete(GrRenderTarget* renderTarget) {
    ...
    GrGLRenderTarget* rt = ((GrGLRenderTarget*)renderTarget);
    if (rt->textureFBOID()) {
        fRenderTargetCnt--;
    }
    if (rt->renderFBOID() && rt->renderFBOID() != rt->textureFBOID()) {
        fRenderTargetCnt--;
    }
