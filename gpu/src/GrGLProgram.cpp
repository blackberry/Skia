...
void GrGLProgram::genStageCode(int stageNum,
    ...
    if (ProgramDesc::StageDesc::k2x2_FetchMode == desc.fFetchMode) {
        GrAssert(GrSamplerState::kRepeat_WrapMode != desc.fWrapModeHorizontal &&
                 GrSamplerState::kRepeat_WrapMode != desc.fWrapModeVertical);
        ...
        GrStringBuilder texXCoord;
        GrStringBuilder texYCoord;

        // Call fract() to simulate GL_REPEAT
        if (GrSamplerState::kRepeat_WrapMode == desc.fWrapModeHorizontal)
            texXCoord.appendf("fract(%s.x)", sampleCoords.c_str());
        else
            texXCoord.appendf("%s.x", sampleCoords.c_str());

        if (GrSamplerState::kRepeat_WrapMode == desc.fWrapModeVertical)
            texYCoord.appendf("fract(%s.y)", sampleCoords.c_str());
        else
            texYCoord.appendf("%s.y", sampleCoords.c_str());

        segments->fFSCode.appendf("\t%s = %s(%s, vec2(%s, %s))%s%s;\n", fsOutColor, texFunc.c_str(), samplerName.c_str(), texXCoord.c_str(), texYCoord.c_str(), smear, modulate.c_str());
    }
}
